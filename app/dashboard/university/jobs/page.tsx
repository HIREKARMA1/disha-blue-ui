...
const fetchJobs = async (): Promise<void> => {
 try {
 setLoading(true)
 const response = await legacyUniversityApi.getUniversityJobs()
...
const handleApproveJob = async (job: UniversityJob) => {
 ...
 await legacyUniversityApi.approveUniversityJob(job.id)
...
const handleRejectJob = async (job: UniversityJob) => {
 ...
 await legacyUniversityApi.rejectUniversityJob(job.id)
...
const handleConfirmDelete = async () => {
 ...
 try {
 setIsDeleting(true)
 await legacyUniversityApi.deleteJobUniversity(jobToDelete.id)
...
const handleJobUpdated = () => {
 ...
 {/* Header */}
 <div className="rounded-none-none p-6 border border-primary-200 mb-6">
 ...
 {/* Search and Filters */}
 <div className="bg-white rounded-none-none border border-gray-200 mb-6 p-6">
 {/* Search Bar */}
 <div className="flex flex-col sm:flex-row gap-3 mb-4">
 <div className="flex-1 relative">
 <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-4 h-4"/>
 <Input
 type="text"placeholder="Search jobs by title, company, or location..."value={searchTerm}
 ...
 className="flex items-center gap-2 border-gray-200 hover:border-gray-300 transition-all duration-200 hover:shadow-md w-full sm:w-auto">
 ...
 <Input
 placeholder="City, State"value={filters.status}
 ...
 <div>
 {/* Results Summary */}
 <div className="bg-white rounded-none-none border border-gray-200 p-4 mb-6">
 ...
 {[...Array(9)].map((_, i) => (
 <div key={i} className="bg-white rounded-none-none border border-gray-200 p-6 animate-pulse">
 <div className="h-4 bg-gray-200 rounded-none-none mb-3"></div>
 <div className="h-6 bg-gray-200 rounded-none-none mb-2"></div>
 <div className="h-4 bg-gray-200 rounded-none-none mb-4"></div>
 <div className="space-y-2">
 <div className="h-3 bg-gray-200 rounded-none-none"></div>
 <div className="h-3 bg-gray-200 rounded-none-none w-3/4"></div>
 </div>
 </div>
 ))}
 </div>
...
const response = await apiClient.getJobById(job.id)
 ...
 ?'Try adjusting your search criteria or filters':'No jobs have been assigned to your university yet'}
 </p>
 <Button
 onClick={clearFilters}
 variant="outline"className="border-gray-200 hover:border-gray-300 transition-all duration-200 hover:shadow-md px-6 py-2">
  Clear Filters
 </Button>
 </div>
 )}
 ...
 title="Not Approve Job"message={`Are you sure you want to not approve this job? This action will remove the job from your university's job list and cannot be undone.`}
 confirmText="Not Approve"cancelText="Cancel"isLoading={jobToReject ? processingJobs.has(jobToReject.id) : false}
 variant="danger"/>
