...
export function MinimalClassicResumeBuilder({
  ...
  initialModel: MinimalClassicModel | null
}) {
  const copy = minimalClassicEn
  const { profile, loading: profileLoading, error: profileError, isAuthenticated, refreshProfile } =
    useProfile()
  const [model, setModel] = useState<MinimalClassicModel | null>(null)
  ...
  useEffect(() => {
    ...
    const base = cloneDefaultsFromJson()
    setModel(mergeProfileIntoMinimalClassic(base, profile as unknown as Record<string, unknown>))
  }, [profile, initialModel])

  const buildStoredContent = useCallback(
    (m: MinimalClassicModel): MinimalClassicResumeContent => ({
      builderVariant: MINIMAL_CLASSIC_BUILDER_VARIANT,
      minimalClassic: m,
    }),
    []
  )
  ...
  const handleAddToProfile = async () => {
    ...
    try {
      ...
      const skillLine = model.skills.filter(Boolean).join(", ")
      const payload: Record<string, string | undefined> = {
        name: model.personalInfo.fullName,
        phone: model.personalInfo.phone || undefined,
        city: model.personalInfo.location || undefined,
        bio: model.aboutMe || undefined,
        institution: firstEdu?.organization,
        degree: firstEdu?.title,
        technical_skills: skillLine || undefined,
        internship_experience: firstExp
          ? `${firstExp.title} at ${firstExp.organization}: ${firstExp.body}`
          : undefined,
      }
    ...
    <div className="min-h-0">
      ...
      <div className="w-full px-3 py-4 sm:px-4 sm:py-6">
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          ...
          >
            <MinimalClassicEditorPanel copy={copy} model={model} onChange={setModel} onAiTarget={openAi} />
          </motion.div>
          <motion.div
            initial={{ opacity: 0, x: 12 }}
            animate={{ opacity: 1, x: 0 }}
          ...
          >
            <div ref={previewRef} className="rounded-xl bg-slate-100/80 p-3 dark:bg-emerald-950/40">
              <MinimalClassicTemplate copy={copy} model={model} />
            </div>
          </motion.div>
        </div>
      </div>
