...
import {
  ...
  type MarcelineModel,
  type MarcelineResumeContent,
} from "@/lib/designerVariantsResume"
import { resumeService } from "@/services/resumeService"
import { useProfile } from "@/hooks/useProfile"
import { MarcelineTemplate } from "./MarcelineTemplate"
import { MarcelineEditorPanel, type MarcelineAiTarget } from "./MarcelineEditorPanel"
import { SectionAiEnhanceModal } from "../minimal-classic/SectionAiEnhanceModal"
import { cloneMarcelineDefaults, mergeProfileIntoMarceline } from "./buildInitialModel"
...
function getDraftTextForMarcelineAi(model: MarcelineModel, target: MarcelineAiTarget | null): string {
  if (!target) return ""
...
function applyAiTextToMarcelineModel(
...
export function MarcelineResumeBuilder({
  ...
  initialModel: MarcelineModel | null
}) {
  const copy = marcelineEn
  const { profile, loading: profileLoading, error: profileError, isAuthenticated, refreshProfile } =
    useProfile()
  const [model, setModel] = useState<MarcelineModel | null>(null)
  ...
  const [aiTarget, setAiTarget] = useState<MarcelineAiTarget | null>(null)
  ...
  useEffect(() => {
    ...
    const base = cloneMarcelineDefaults()
    setModel(mergeProfileIntoMarceline(base, profile as unknown as Record<string, unknown>))
  }, [profile, initialModel])

  const buildStoredContent = useCallback(
    (m: MarcelineModel): MarcelineResumeContent => ({
      builderVariant: MARCELINE_BUILDER_VARIANT,
      marcelineSingle: m,
    }),
    []
  )
  ...
  const handleAddToProfile = async () => {
    ...
    try {
      ...
      const firstExp = model.experience[0]
      const skillLine = model.skillGroups.map((g) => g.itemsLine).filter(Boolean).join(", ")
      const payload: Record<string, string | undefined> = {
        name: model.personalInfo.fullName,
        phone: model.personalInfo.phone || undefined,
        city: model.personalInfo.addressLine || undefined,
        bio: model.profileSummary || undefined,
        institution: firstEdu?.institutionYear?.split(",")[0]?.trim(),
        degree: firstEdu?.degree,
        technical_skills: skillLine || undefined,
        internship_experience: firstExp
          ? `${firstExp.role} at ${firstExp.company}: ${firstExp.bullets.join(" ")}`
          : undefined,
      }
  ...
  const openAi = (target: MarcelineAiTarget, _currentText: string, sectionLabel: string) => {
    setAiTarget(target)
  ...
  const runAi = async (optionalHint: string) => {
    ...
    const draftText = getDraftTextForMarcelineAi(model, aiTarget)
    try {
      const res = await runLakshyaEnhance({
        ...
        context: {
          ...
          templateId: MARCELINE_BUILDER_VARIANT,
        },
        language: locale,
      })
      if (!res.success) {
      ...
      setModel(applyAiTextToMarcelineModel(model, aiTarget, res.enhancedText))
      setAiOpen(false)
    ...
    <div className="min-h-0">
      ...
      <div className="w-full px-3 py-4 sm:px-4 sm:py-6">
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          ...
          >
            <MarcelineEditorPanel copy={copy} model={model} onChange={setModel} onAiTarget={openAi} />
          </motion.div>
          <motion.div
            initial={{ opacity: 0, x: 12 }}
            animate={{ opacity: 1, x: 0 }}
          ...
          >
            <div ref={previewRef} className="rounded-xl bg-slate-100/80 p-3 dark:bg-emerald-950/40">
              <MarcelineTemplate copy={copy} model={model} />
            </div>
          </motion.div>
        </div>
      </div>
