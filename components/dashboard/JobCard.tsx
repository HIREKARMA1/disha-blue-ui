...
export function JobCard({
    ...
    const getApplicationStatusDisplay = (status: string) => {
        switch (status) {
            case 'applied':
                return (
                    <span className="inline-flex items-center justify-center gap-1 rounded-full bg-primary/10 px-2.5 py-1 text-xs font-semibold text-primary">
            ...
            case 'shortlisted':
                return (
                    <span className="inline-flex items-center justify-center gap-1 rounded-full bg-secondary/15 px-2.5 py-1 text-xs font-semibold text-secondary">
            ...
            case 'selected':
                return (
                    <span className="inline-flex items-center justify-center gap-1 rounded-full bg-emerald-500/15 px-2.5 py-1 text-xs font-semibold text-emerald-700 dark:text-emerald-400">
                        <CheckCircle className="h-3 w-3" />
                        Selected
                    </span>
                )
            case 'rejected':
                return (
                    <span className="inline-flex items-center justify-center gap-1 rounded-full bg-destructive/10 px-2.5 py-1 text-xs font-semibold text-destructive">
                        <X className="h-3 w-3" />
                        Not selected
                    </span>
                )
            case 'pending':
                return (
                    <span className="inline-flex items-center justify-center gap-1 rounded-full bg-muted px-2.5 py-1 text-xs font-semibold text-muted-foreground">
    ...
    return (
        <motion.article
            ...
            className={cn(
                'group flex h-full flex-col overflow-hidden rounded-2xl border border-border/80 bg-card shadow-[0_20px_50px_-28px_hsl(var(--primary)/0.28)] transition-all duration-300',
                'hover:border-primary/25 hover:shadow-[0_24px_64px_-28px_hsl(var(--primary)/0.35)] dark:shadow-[0_20px_50px_-28px_rgba(0,0,0,0.45)]',
            )}
        >
            <div className="relative border-b border-border/60 bg-muted/20 px-5 pb-4 pt-5">
                <div className="flex gap-4">
                    <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-primary-500/90 to-secondary-500/90 text-sm font-bold text-white shadow-md shadow-primary/20">
                        {initials.slice(0, 3)}
                    </div>
                    <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-3 flex flex-wrap items-center gap-2">
                            {isOnCampusJob() && (
                                <span className="rounded-full border border-primary/25 bg-primary/5 px-2.5 py-0.5 text-[11px] font-semibold uppercase tracking-wide text-primary">
                                    On campus
                                </span>
                            )}
                            <span className="rounded-full border border-border bg-background/80 px-2.5 py-0.5 text-[11px] font-semibold text-foreground/80">
            ...
            <div className="flex flex-1 flex-col px-5 py-4">
                    ...
                    <div className="mt-4 flex flex-wrap gap-1.5">
                        {job.skills_required.slice(0, 4).map((skill, index) => {
                            ...
                            return (
                                <span
                                    key={index}
                                    className="rounded-lg border border-border/70 bg-muted/40 px-2 py-0.5 text-[11px] font-medium text-foreground/85"
                                >
                                    {skillText}
                                </span>
                            )
                        ...
                        {job.skills_required.length > 4 && (
                            <span className="rounded-lg border border-dashed border-border px-2 py-0.5 text-[11px] text-muted-foreground">
                                +{job.skills_required.length - 4}
                            </span>
                        )}
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted-foreground">
                        {job.application_deadline && (
                            <span className="inline-flex items-center gap-1">
                                ...
                                {isDeadlineNear() && (
                                    <span className="font-semibold text-amber-600 dark:text-amber-400"> · Soon</span>
                                )}
                                {isDeadlineExpired() && (
                ...
                <div className="mt-4 flex gap-2">
                    <Button
                        ...
                        variant="outline"
                        className="h-10 flex-1 rounded-xl border-border/80 font-semibold"
                    >
                        View role
                    </Button>
                    <Button
