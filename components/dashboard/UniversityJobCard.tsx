...
const getCardColorScheme = (index: number) => {
 ...
 return approved
 ?'bg-blue-50 text-blue-800':'bg-yellow-50 text-yellow-800'}
 ...
 {showDropdown && (
 <div className="absolute right-0 top-8 z-50 w-48 bg-white border border-gray-200 rounded-xl shadow-lg">
 <div className="py-1">
 {onViewApplications && (
 <button
 onClick={() => {
...
const skillText = typeof skill ==='string'? skill : String(skill ||'')
 ...
 <span
 key={index}
 className="px-2 py-1 text-xs bg-gray-50 text-gray-700 rounded-xl">
 {skillText}
 </span>
 )
 })}
 {job.skills_required.length > 3 && (
 <span className="px-2 py-1 text-xs bg-gray-50 text-gray-700 rounded-xl">
 ...
 size="sm"className="w-full sm:flex-1 sm:min-w-[90px] flex items-center justify-center gap-1.5 text-xs bg-blue-500 hover:bg-blue-600 text-white transition-all duration-200 hover:shadow-md">
 {isProcessing ? (
 <>
