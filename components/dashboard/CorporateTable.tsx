...
export function CorporateTable({
    ...
    const getStatusColor = (status: string) => {
        switch (status) {
            case 'active':
                return 'border-green-200/60 bg-green-500/10 text-green-700 dark:text-green-300'
            case 'inactive':
                return 'border-amber-200/60 bg-amber-500/10 text-amber-700 dark:text-amber-300'
            case 'suspended':
                return 'border-red-200/60 bg-red-500/10 text-red-700 dark:text-red-300'
            default:
    ...
    const getVerifiedColor = (verified: boolean) => {
        return verified
            ...
            : 'border-amber-200/60 bg-amber-500/10 text-amber-700 dark:text-amber-300'
    }
    ...
    if (isLoading) {
        return (
            <div className="rounded-2xl border border-border bg-card/80 overflow-hidden shadow-soft">
        ...
        return (
            <div className="rounded-2xl border border-border bg-card/80 overflow-hidden shadow-soft">
                <div className="p-8 text-center">
                    ...
                    <button
                        onClick={onRetry}
                        className="px-4 py-2 bg-primary text-primary-foreground rounded-lg hover:opacity-90 transition-colors duration-200"
                    >
                        Try Again
                    </button>
                </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full">
                        <thead className="bg-muted/40 border-b border-border">
                            <tr>
                                <th
                                    className="px-6 py-4 text-left text-xs font-semibold text-muted-foreground uppercase tracking-[0.14em] cursor-pointer hover:bg-muted/60 transition-colors duration-200"
                        ...
                        <tbody className="bg-card divide-y divide-border/70">
                            {paginatedCorporates.map((corporate, index) => (
                                <motion.tr
                                    key={corporate.id}
                                    className="hover:bg-muted/40 transition-colors duration-200 cursor-pointer"
                                ...
                                >
                                    ...
                                    {/* Verification Status */}
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <span className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-medium ${getVerifiedColor(corporate.verified)}`}>
                                    ...
                                    {/* Status */}
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <span className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-medium ${getStatusColor(corporate.status)}`}>
                                            {corporate.status}
                                        </span>
                                    </td>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div className="flex items-center justify-end space-x-2">
                                            <button
                                                ...
                                                className="p-2 rounded-lg transition-colors duration-200 text-primary hover:bg-primary/10"
                                            ...
                                            <button
                                                ...
                                                className="p-2 rounded-lg transition-colors duration-200 text-destructive hover:bg-destructive/10"
                        ...
                        <div className="flex items-center justify-between">
                            <div className="flex-1 flex justify-between sm:hidden">
                                ...
                                <button
                                    ...
                                    className="ml-3 relative inline-flex items-center rounded-md border border-border bg-background px-4 py-2 text-sm font-medium text-foreground hover:bg-muted disabled:opacity-50 disabled:cursor-not-allowed"
                                >
                                    Next
                                </button>
                            </div>
                                ...
                                <div>
                                    <nav className="relative z-0 inline-flex rounded-md -space-x-px">
                                        <button
                                            onClick={() => setCurrentPage(prev => Math.max(prev - 1, 1))}
                                            disabled={currentPage === 1}
                                            className="relative inline-flex items-center rounded-l-md border border-border bg-background px-2 py-2 text-sm font-medium text-muted-foreground hover:bg-muted disabled:opacity-50 disabled:cursor-not-allowed"
                                        >
                                            Previous
                                        </button>
                                        {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                                            <button
                                                ...
                                                className={`relative inline-flex items-center px-4 py-2 border text-sm font-medium ${page === currentPage
                                                    ? 'z-10 border-primary/40 bg-primary/10 text-primary'
                                        ...
                                        <button
                                            onClick={() => setCurrentPage(prev => Math.min(prev + 1, totalPages))}
                                            disabled={currentPage === totalPages}
                                            className="relative inline-flex items-center rounded-r-md border border-border bg-background px-2 py-2 text-sm font-medium text-muted-foreground hover:bg-muted disabled:opacity-50 disabled:cursor-not-allowed"
                                        >
                                            Next
                                        </button>
                                    </nav>
