...
import { useIndustries } from '@/hooks/useLookup'// Degree options (same as student modal)
const degreeOptions = [
...
const normalizeSelectValue = (value: string | null | undefined, options: { value: string }[]): string => {
 ...
 console.warn(` Value"${trimmed}"does not match any option. Available options:`, options.map(o => o.value))
 ...
 block_or_tehsil: job.block_or_tehsil ||'',
 city_or_town: job.city_or_town ||'',
 village_or_locality: job.village_or_locality ||'',
 pincode: job.pincode ||'',
...
const previewUrl = URL.createObjectURL(file)
 ...
 block_or_tehsil: formData.block_or_tehsil || null,
 city_or_town: formData.city_or_town || null,
 village_or_locality: formData.village_or_locality || null,
 ...
 animate={{ opacity: 1 }}
 exit={{ opacity: 0 }}
 className={cn(corporateModalBackdropClass,'z-[100]')}
 onClick={onClose}
 >
 <motion.div
 ...
 className={cn(corporateModalShellClass,'max-w-4xl w-full max-h-[90vh]')}
 ...
 <div>
 <h2 className="font-display text-2xl font-semibold tracking-tight text-foreground">Edit job posting</h2>
 <p className="text-muted-foreground text-sm mt-1 max-w-xl">Update details—the same save flow and validation as before.</p>
 </div>
 <button
 ...
 placeholder="List the required qualifications, skills, and experience..."rows={3}
 />
 </div>
 ...
 placeholder="Describe the key responsibilities and duties..."rows={3}
 />
 </div>
 </div>
 ...
 placeholder="Upload company logo"className="w-full"/>
 {uploadingLogo && (
 ...
 <Input
 type="number"value={formData.company_founded}
 onChange={(e) => handleInputChange('company_founded', e.target.value)}
 placeholder="e.g., 2000"min="1900"max={new Date().getFullYear()}
 />
 </div>
 </div>
 ...
 placeholder="Describe the company, its mission, values, and what makes it special..."rows={4}
 />
 </div>
 ...
 Keep state, district, and city/town accurate for better hyper-local job matching.
 </p>
 ...
 placeholder={validationErrors.location ||"Add legacy location text (e.g., Bhubaneswar, Odisha)"}
 ...
 <span
 key={index}
 className="inline-flex items-center gap-1 px-3 py-1 bg-blue-100 text-blue-800 text-sm">
 {location}
 <button
 type="button"onClick={() => removeLocation(location)}
 ...
 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
 <Input value={formData.state} onChange={(e) => handleInputChange('state', e.target.value)} placeholder="State"/>
 <Input value={formData.district} onChange={(e) => handleInputChange('district', e.target.value)} placeholder="District"/>
 <Input value={formData.city_or_town} onChange={(e) => handleInputChange('city_or_town', e.target.value)} placeholder="City / Town"/>
 <Input value={formData.village_or_locality} onChange={(e) => handleInputChange('village_or_locality', e.target.value)} placeholder="Village / Locality"/>
 <Input value={formData.block_or_tehsil} onChange={(e) => handleInputChange('block_or_tehsil', e.target.value)} placeholder="Block / Tehsil (optional)"/>
 <Input value={formData.pincode} onChange={(e) => handleInputChange('pincode', e.target.value)} placeholder="Pincode"/>
 <Input value={formData.address_line} onChange={(e) => handleInputChange('address_line', e.target.value)} placeholder="Address line"/>
 <Input value={formData.landmark} onChange={(e) => handleInputChange('landmark', e.target.value)} placeholder="Landmark"/>
 </div>

 <div className="flex items-center space-x-6 flex-wrap gap-4">
 <label className="flex items-center space-x-2">
 <input
 type="checkbox"checked={formData.remote_work}
 onChange={(e) => handleInputChange('remote_work', e.target.checked)}
 className="rounded-none border-gray-300 text-primary-600 focus:ring-primary-500"/>
 <span className="text-sm text-gray-700">Remote work allowed</span>
 </label>

 <label className="flex items-center space-x-2">
 <input
 type="checkbox"checked={formData.travel_required}
 ...
 <label className="flex items-center space-x-2">
 <input
 type="checkbox"checked={formData.onsite_office}
 onChange={(e) => handleInputChange('onsite_office', e.target.checked)}
 className="rounded-none border-gray-300 text-primary-600 focus:ring-primary-500"/>
 <span className="text-sm text-gray-700">Onsite office</span>
 </label>
 </div>
 </div>
 ...
 <Input
 type="number"min="0"value={formData.salary_min}
 onChange={(e) => {
 ...
 placeholder="e.g., 500000"className="[&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none [&::-moz-appearance]:textfield"/>
 </div>
 ...
 <Input
 type="number"min="0"value={formData.salary_max}
 onChange={(e) => {
 const value = e.target.value;
 if (value ===''|| (!isNaN(parseFloat(value)) && parseFloat(value) >= 0)) {
 ...
 placeholder="e.g., 800000"className="[&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none [&::-moz-appearance]:textfield"/>
 </div>
 ...
 <Input
 type="number"min="0"value={formData.experience_min}
 onChange={(e) => {
 const value = e.target.value;
 ...
 placeholder="e.g., 2"className="[&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none [&::-moz-appearance]:textfield"/>
 </div>
 ...
 <Input
 type="number"min="0"value={formData.experience_max}
 onChange={(e) => {
 const value = e.target.value;
 if (value ===''|| (!isNaN(parseInt(value)) && parseInt(value) >= 0)) {
 ...
 placeholder="e.g., 5"className="[&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none [&::-moz-appearance]:textfield"/>
 </div>
 ...
 <span
 key={index}
 className="inline-flex items-center gap-1 px-3 py-1 bg-primary-100 text-primary-800 text-sm">
 ...
 level ==='phd'?'PhD': level}
 <button
 type="button"onClick={() => handleMultiSelectChange('education_level', level)}
 ...
 placeholder="Add a skill (e.g., Python, React, AWS)"onKeyPress={(e) => e.key ==='Enter'&& (e.preventDefault(), addSkill())}
 />
 ...
 <button
 type="button"onClick={() => handleMultiSelectChange('education_degree', degree)}
 className="text-primary-600 hover:text-primary-800">
 <X className="w-3 h-3"/>
 </button>
 </span>
 ...
 <button
 type="button"onClick={() => handleMultiSelectChange('education_branch', branch)}
 className="text-primary-600 hover:text-primary-800">
 <X className="w-3 h-3"/>
 </button>
 </span>
 ...
 placeholder="e.g., B.Tech, MCA"rows={2}
 />
 </div>
 ...
 placeholder="e.g., Unlimited growth opportunities, Highly competitive pay, Learning and development"rows={3}
 />
 </div>
 ...
 placeholder="Enter service agreement details if applicable"rows={2}
 />
 </div>
 </div>
 ...
 placeholder="Select application deadline"autoClose={true}
 />
 </div>
 ...
 <Input
 type="number"value={formData.number_of_openings}
 onChange={(e) => handleInputChange('number_of_openings', e.target.value)}
 placeholder="e.g., 3"className={validationErrors.number_of_openings ?"border-red-500 placeholder-red-500":""}
 />
 ...
 placeholder="Select expiration date"autoClose={true}
 />
 </div>
 </div>
 ...
 placeholder="Round 1: Online Test, Round 2: Technical Interview, Round 3: HR Round"rows={2}
 />
 <p className="text-xs text-gray-500 mt-1">
 ...
 placeholder="Select campus drive date"autoClose={true}
 />
 </div>
 </div>
 </div>
