...
export function CorporateJobCard({ job, onViewDescription, onEdit, onDelete, onStatusChange, onViewAppliedStudents, cardIndex: _cardIndex = 0 }: CorporateJobCardProps) {
 ...
 const colors = {
 full_time:'text-primary',
 part_time:'text-secondary-foreground',
 contract:'bg-muted text-foreground border-border',
 internship:'text-primary border-border',
 freelance:'text-foreground border-border',
 }
 ...
 <span className={cn('border px-2.5 py-0.5 text-xs font-medium',
 ...
 className="flex w-full items-center gap-2 px-4 py-2.5 text-left text-sm text-foreground">
 ...
 className="flex w-full items-center gap-2 px-4 py-2.5 text-left text-sm text-destructive">
 ...
 return (
 <span className={cn(corporateChipClass,'shrink-0')}>Remote</span>
 )
 }
 } else {
...
const skillText = typeof skill ==='string'? skill : String(skill ||'')
 ...
 <span className={cn(corporateChipClass,'')}>
