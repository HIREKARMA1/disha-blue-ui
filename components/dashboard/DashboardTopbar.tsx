...
export function DashboardTopbar({ role }: DashboardTopbarProps) {
  ...
  return (
    <section className="mb-6 rounded-2xl border border-border/70 bg-card/80 p-4 shadow-soft backdrop-blur-sm sm:p-5">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          <p className="text-xs font-semibold uppercase tracking-[0.16em] text-primary">
            {role === "student"
              ? t(locale, "dashboard.labels.studentWorkspace")
              : role === "corporate"
                ? t(locale, "dashboard.labels.recruiterWorkspace")
                : t(locale, "dashboard.labels.adminWorkspace")}
          </p>
          <h1 className="mt-2 font-display text-2xl font-semibold tracking-tight text-foreground sm:text-[1.7rem]">
            {title}
          </h1>
