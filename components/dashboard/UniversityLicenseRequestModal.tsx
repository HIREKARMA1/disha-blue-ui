...
const checkEligibility = async (
 ...
 const normalizedDegrees = candidateDegrees
...
const errorMessage = error.response?.data?.detail ||
 ...
 error.message || 'Unable to verify eligibility automatically'
...
const isRenewal = eligibility?.request_type ==='RENEWAL'|| isRenewalFlow
 ...
 const degreesList = formData.degree
...
const today = new Date()
 ...
 animate={{ opacity: 1 }}
 exit={{ opacity: 0 }}
 className="fixed inset-0 bg-black z-[9999] flex items-center justify-center p-4"onClick={onClose}
 >
 <motion.div
 ...
 ?"You are requesting a renewal for an exhausted batch. This will be reviewed by the admin.":"Submit a request for student licenses. The admin will review your request and approve the licenses."}
 </p>
 </div>
 ...
 <Input
 type="text"value={formData.batch}
 onChange={handleBatchChange}
 placeholder="e.g. 2024-2025"className={`pl-9 bg-gray-100 ${eligibility && !eligibility.eligible
 ?'border-red-300 focus:ring-red-500':''}`}
 />

 {checkingEligibility && (
 <div className="absolute right-3 top-2.5">
 <div className="animate-spin h-4 w-4 border-2 border-blue-500 border-t-transparent"></div>
 </div>
 )}
 </div>
 </div>
 ...
 <Input
 type="number"min="1"max="1000"value={formData.requested_total}
 onChange={(e) => setFormData(prev => ({ ...prev, requested_total: e.target.value }))}
 placeholder="e.g. 100"className="pl-9"/>
 </div>
 </div>
 <MultiSearchableSelect
 label="Degree (Optional)"values={formData.degree}
 onChange={handleDegreeChange}
 options={degreeOptions}
 placeholder="Select degrees"searchPlaceholder="Search degrees..."/>
 <MultiSearchableSelect
 label="Branches (Optional)"options={branchDropdownOptions}
 ...
 <Input
 type="date"value={formData.period_from}
 ...
 <Calendar className="absolute left-3 top-2.5 h-4 w-4 text-gray-400"/>
 <Input
 type="date"value={formData.period_to}
