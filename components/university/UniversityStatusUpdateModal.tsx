...
interface UniversityStatusUpdateModalProps {
  isOpen: boolean
  onClose: () => void
...
export function UniversityStatusUpdateModal({
  isOpen,
  onClose,
  application,
  onSuccess
}: UniversityStatusUpdateModalProps) {
  ...
  setNotes(application.corporate_notes || '')
  ...
  await apiClient.updateUniversityApplicationStatus(
  application.id,
  status,
  notes || undefined,
  ...
  toast.success('Application status updated and offer letter uploaded successfully!')
  } else {
  ...
  const errorMessage = error.response?.data?.detail || 'Failed to update application status'
  ...
  {/* Interview Details - Only show for shortlisted */}
  <AnimatePresence>
