...
export function PracticeDashboard() {
  ...
  return (
  <div className="space-y-6">
  {/* Header — dashboard card + sage tint (library / video-search) */}
  ...
  <span className="inline-flex items-center rounded-full border border-sage/40 bg-white px-3 py-1 text-sm font-medium text-sage-deep dark:border-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-200">
  ...
  icon: Brain,
  iconWrap: 'bg-sage/25 dark:bg-emerald-900/50',
  color: 'text-sage-deep dark:text-emerald-300',
  },
  {
  label: 'Total Questions',
  value: filteredModules.reduce((sum, module) => sum + module.questions_count, 0).toString(),
  icon: Clock,
  iconWrap: 'bg-emerald-100/80 dark:bg-emerald-900/45',
  ...
  icon: Trophy,
  iconWrap: 'bg-sage/20 dark:bg-emerald-900/40',
  color: 'text-sage-deep dark:text-emerald-200',
  },
  ].map((stat, index) => (
  <motion.div
  key={stat.label}
  ...
  <div className="dashboard-overview-card-interactive w-full p-4 transition-all duration-200 hover:-translate-y-1">
  <div className="flex items-center justify-between">
  <div className="flex-1">
  <p className="mb-1 text-sm font-medium text-slate-600 dark:text-emerald-200/80">
  {stat.label}
  </p>
  <p className="text-2xl font-bold text-slate-900 transition-transform duration-200 group-hover:scale-105 dark:text-emerald-50">
  {isLoading ? (
  <div className="h-8 w-16 animate-pulse rounded-lg bg-slate-200/90 dark:bg-emerald-800/50"></div>
  ) : (
  stat.value
  )}
  </p>
  </div>
  <div className={`rounded-xl p-3 shadow-sm transition-transform duration-200 group-hover:scale-110 ${stat.iconWrap}`}>
  ...
  <div className="dashboard-overview-card overflow-hidden p-0 transition-shadow duration-300 hover:shadow-md">
  <div className="flex items-center gap-2 border-b border-slate-200/90 p-6 dark:border-emerald-800/70">
  <Briefcase className="h-5 w-5 text-sage-deep dark:text-emerald-300" />
  <h2 className="text-xl font-semibold text-slate-900 dark:text-emerald-50">
  Hiring Assessments
  </h2>
  </div>
  <div className="p-6">
  ...
  <div className="dashboard-overview-card overflow-hidden p-0 transition-shadow duration-300 hover:shadow-md">
  <div className="border-b border-slate-200/90 p-6 dark:border-emerald-800/70">
  <h2 className="text-xl font-semibold text-slate-900 dark:text-emerald-50">
  ...
  <div className="rounded-2xl border border-slate-200/90 bg-slate-50/80 p-12 text-center dark:border-emerald-800/60 dark:bg-emerald-950/30">
  <Brain className="mx-auto mb-4 h-16 w-16 text-slate-400 dark:text-emerald-500/70" />
  ...
  variant="outline"
  className="mt-4 border-slate-200/90 transition-all duration-200 hover:bg-sage/10 dark:border-emerald-800 dark:text-emerald-100 dark:hover:bg-emerald-900/40"
  >
  Clear Filters
  </Button>
  )}
