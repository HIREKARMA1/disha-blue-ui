...
export function ProfileCompletion({ completion, fields, completionData, className }: ProfileCompletionProps) {
  ...
  if (percentage >= 80) return 'bg-blue-600 dark:bg-blue-500'
  if (percentage >= 60) return 'bg-blue-600 dark:bg-blue-400'
  ...
  return (
  <div className={cn("rounded-xl border border-slate-200/90 bg-white p-6 shadow-sm dark:border-blue-800/70 dark:bg-blue-900/25", className)}>
  {/* Header */}
  <div className="flex items-center justify-between mb-6">
  <div>
  <h3 className="text-lg font-semibold text-slate-900 dark:text-blue-50">
  Profile Completion
  </h3>
  <p className="text-sm text-slate-600 dark:text-blue-200/80">
  ...
  <div className="text-xs text-slate-500 dark:text-blue-400/90">
  ...
  {/* Progress Bar */}
  <div className="mb-6">
  <div className="h-3 w-full rounded-full bg-slate-200 dark:bg-blue-950/60">
  <div
  className={cn("h-3 rounded-full transition-all duration-500 ease-out", getProgressColor(completion))}
  style={{ width: `${completion}%` }}
  />
  </div>
  <div className="mt-2 flex justify-between text-xs text-slate-500 dark:text-blue-400/80">
  <span>0%</span>
  <span>25%</span>
  <span>50%</span>
  <span>75%</span>
  ...
  <h4 className="text-sm font-medium text-slate-900 dark:text-blue-50">
  Category Progress
  </h4>
  {categoryStats.map(({ category, completed, total, percentage }) => (
  <div key={category} className="space-y-2">
  <div className="flex items-center justify-between">
  <span className="text-sm capitalize text-slate-700 dark:text-blue-200/90">
  {category}
  </span>
  <span className="text-sm text-slate-600 dark:text-blue-200/75">
  {completed}/{total}
  </span>
  </div>
  <div className="h-2 w-full rounded-full bg-slate-200 dark:bg-blue-950/60">
  <div
  ...
  return (
  <div key={index} className="rounded bg-blue-50/15 px-2 py-1 text-xs text-slate-700 dark:bg-blue-900/40 dark:text-blue-200/90">
  {formatted}
  </div>
  )
  })}
  {completedFieldsList.length > 6 && (
  <div className="px-2 py-1 text-xs text-slate-500 dark:text-blue-400/80">
  ...
  {/* Completion Tips */}
  {completion < 100 && (
  <div className="mt-6 rounded-lg border border-blue-50/40 bg-blue-50/10 p-4 dark:border-blue-800 dark:bg-blue-900/35">
  <h5 className="mb-2 text-sm font-medium text-blue-600 dark:text-blue-200">
  Tips to Complete Your Profile
  </h5>
  <ul className="space-y-1 text-xs text-slate-700 dark:text-blue-200/85">
