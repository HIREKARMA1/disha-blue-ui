...
import {
  ...
  useCreateCorporatePracticeModule,
  useCorporateQuestions,
  useCreateCorporatePracticeQuestion,
  useUpdateCorporatePracticeQuestion,
  useAddQuestionToModule,
  useDeleteCorporatePracticeQuestion,
  useRemoveQuestionFromModule
} from '@/hooks/useCorporatePractice'
import {
  useUpdateCorporatePracticeModule,
  useDeleteCorporatePracticeModule
} from '@/hooks/useCorporatePractice'
...
import { corporateHeroClass } from '@/components/corporate/corporate-ui'
...
type ViewState = 'modules' | 'questions' | 'module-detail'
...
export function CorporatePracticeManager() {
  ...
  // API hooks
  const { data: modules, isLoading: modulesLoading, error: modulesError, refetch: refetchModules } = useCorporatePracticeModules(
  ...
  const createModuleMutation = useCreateCorporatePracticeModule()
  const createQuestionMutation = useCreateCorporatePracticeQuestion()
  const updateQuestionMutation = useUpdateCorporatePracticeQuestion()
  const addQuestionToModuleMutation = useAddQuestionToModule()

  // Action hooks
  const updateModuleMutation = useUpdateCorporatePracticeModule()
  const deleteModuleMutation = useDeleteCorporatePracticeModule()
  const deleteQuestionMutation = useDeleteCorporatePracticeQuestion()
  ...
  setIsQuestionModalOpen(true)
  console.log(' Opening question modal for editing')
  }
  ...
  const newUrl = `${window.location.pathname}?moduleId=${module.id}`
  ...
  setAttemptsModule(module)
  setIsAttemptsModalOpen(true)
  // Update URL with module ID and view parameter
  const newUrl = `${window.location.pathname}?moduleId=${module.id}&view=attempts`
  window.history.pushState({}, '', newUrl)
  }
  ...
  console.log(' Delete button clicked for module:', module.title, module.id)
  ...
  onConfirm: async () => {
  try {
  console.log(' Confirming deletion of module:', module.id)
  ...
  onConfirm: async () => {
  try {
  console.log(' Confirming deletion of question:', question.id)
  ...
  console.warn(' Failed to remove from module, continuing with delete:', removeError)
  }
  }
  ...
  <p className="text-muted-foreground text-base mb-3 max-w-2xl">
  Manage practice tests, questions, and student attempts in one recruiter-grade workspace.
  </p>
  ...
  isLoading={deleteModuleMutation.isLoading || deleteQuestionMutation.isLoading}
  />
...
function ModuleDetailView({ module, onBack, onCreateQuestion, onBulkUpload, onEditQuestion, onDeleteQuestion, refreshTrigger, deletedQuestionIds }: ModuleDetailViewProps) {
  ...
  const { data: questions, isLoading: questionsLoading, error: questionsError, refetch: refetchQuestions } = useCorporateQuestions(module.id)
  ...
  <p className="text-muted-foreground text-base mb-3 max-w-2xl">
  Questions and modules for this workspace.
  </p>
  <div className="flex flex-wrap gap-2">
  <span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-primary-100 dark:bg-primary-900/30 text-primary-800 dark:text-primary-200">
  {new Date().toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}
...
function CreateModuleForm({ module, onSave, onCancel }: CreateModuleFormProps) {
  ...
  // Fetch universities
  const { data: universities, isLoading: universitiesLoading, error: universitiesError } = useUniversities()

  // Convert universities to dropdown options
  const universityOptions: MultiSelectOption[] = React.useMemo(() => {
  ...
  // University targeting fields (required by schema)
  university_target_all_branches: formData.branch_ids.length === 0,
  ...
  {/* Header */}
  <div className="relative rounded-2xl border border-border/80 bg-gradient-to-br from-primary/[0.07] via-card to-secondary/[0.08] p-4 shadow-sm">
  <button
  onClick={onCancel}
  ...
  <span
  key={tag}
  className="inline-flex items-center gap-1 px-2.5 py-1 bg-primary-100 dark:bg-primary-900/30 text-primary-800 dark:text-primary-200 rounded-full text-xs"
  >
  {tag}
  <button
