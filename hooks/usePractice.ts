...
function getPracticeApiErrorMessage(err: unknown, fallback: string): string {
  if (isAxiosError(err)) {
  ...
  if (status === 403) {
  return 'You don’t have access to practice modules. Sign in with a student account, or contact your administrator if this persists.'
  }
  }
...
export function usePracticeModules() {
  ...
  if (!isAuthenticated) {
  setData([])
  setError(new Error('Sign in to view practice modules.'))
  setIsLoading(false)
  return
  }
  ...
  setData([])
  setError(new Error('Practice is only available for student accounts. Open the student dashboard with a student login.'))
  setIsLoading(false)
  return
  }
  ...
  setError(new Error(getPracticeApiErrorMessage(err, 'Failed to fetch practice modules')))
...
export function usePracticeQuestions(moduleId: string) {
  ...
  setError(
 !isAuthenticated
  ? new Error('Sign in to load this practice test.')
  : new Error('Practice tests are only available for student accounts.')
  )
  setIsLoading(false)
  return
  }
  ...
  setError(new Error(getPracticeApiErrorMessage(err, 'Failed to fetch questions')))
  ...
  fetchQuestions()
  }, [moduleId, authLoading, isAuthenticated, user?.user_type])
